...
func CreateOctopusAsyncApiClient(
...
func NewOctopusApiClient(taskCtx plugin.TaskContext, connection *models.OctopusConnection) (*api.ApiAsyncClient, errors.Error) {
	apiClient, err := api.NewApiClientFromConnection(taskCtx.GetContext(), taskCtx, connection)
	if err != nil {
		return nil, err
	}

	return CreateOctopusAsyncApiClient(taskCtx, apiClient, connection)
}
